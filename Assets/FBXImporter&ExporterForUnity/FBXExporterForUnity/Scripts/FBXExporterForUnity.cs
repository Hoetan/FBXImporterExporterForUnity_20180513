 ...
 *
 *  	Developed by ほえたん(Hoetan) -- 2017/11/15
...
public class FBXExporterForUnity : MonoBehaviour
{
    ...
    [DllImport("FBXExporterForUnity")]
	public static extern bool FBXExporterSave(string pcName, string pcVersion, int iFileFormat, string pcFileFormat2, bool bEmbedMedia, bool bASCIIFBX);
	[DllImport("FBXExporterForUnity")]
    ...
    public bool bOutBlendShape = true;
		...
		if (bFirst) {
			...
			if (!bSave && bOutFBX) {
                ...
                */
                ...
                FBXExporterSave(strFilename2, strVersions[(int)eFBXExportFileVersion], (int)eFBXFileFormat, (string)strFileFormat2, bEmbedMedia, (eFBXFileFormat == EFBXFileFormat.eASCII));
				...
				if (!dicOutputTextureNames.TryGetValue(strTexture, out strDestPathTexture))
				{
					...
					if (bConvertAllPNG)
					{
                        ...
                        if (importer.textureType == TextureImporterType.NormalMap)
                        {
                            ...
                            if (bDebugLog) Debug.Log("Output PNG Normal Texture:<" + strDestPathTexture + ".png>");
                        }
                        else
                        {
